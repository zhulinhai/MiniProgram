...
function genAcceleratePlayUrl (userid, txTime) {
  ...
  var pushUrl = 'rtmp://' + config.live.bizid + '.liveplay.myqcloud.com/live/' + liveCode + ext
  return pushUrl
}
...
function genPushUrl (userid, txTime) {
  ...
  var pushUrl = 'rtmp://' + config.live.bizid + '.livepush.myqcloud.com/live/' + liveCode + ext
  return pushUrl
}
 ...
 * 生成混流地址
 */
function genMixedPlayUrl (userid, streamType) {
  var liveCode = config.live.bizid + '_' + userid
...
function getStreamIdFromPushUrl (pushUrl) {
  ...
  // console.log("/:", index_2);
  var streamID = substr.substring(index2 + 1, index)
